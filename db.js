...
async function initSchema() {
  ...
  try {
    ...
    await conn.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS session_version INT UNSIGNED NOT NULL DEFAULT 0');
    ...
    await conn.query(`
      CREATE TABLE IF NOT EXISTS performers (
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(255) COLLATE utf8mb4_nopad_bin NOT NULL UNIQUE,
        dir_path TEXT COLLATE utf8mb4_nopad_bin NOT NULL,
    ...
    await conn.query(`
      CREATE TABLE IF NOT EXISTS media (
        ...
        file_path VARCHAR(1000) COLLATE utf8mb4_nopad_bin NOT NULL,
    ...
    // Linux paths distinguish case, accents and trailing spaces. Never rebuild IDs.
    await require('./lib/maintenance').withMaintenance(async () => {
      const [columns] = await conn.query(`
        SELECT TABLE_NAME AS table_name, COLUMN_NAME AS column_name, COLLATION_NAME AS collation_name
        FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE()
        AND ((TABLE_NAME = 'performers' AND COLUMN_NAME IN ('name', 'dir_path'))
          OR (TABLE_NAME = 'media' AND COLUMN_NAME = 'file_path'))
      `);
      for (const column of columns) {
        if (column.collation_name === 'utf8mb4_nopad_bin') continue;
        const definitions = {
          'performers.name': 'ALTER TABLE performers MODIFY name VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_nopad_bin NOT NULL',
          'performers.dir_path': 'ALTER TABLE performers MODIFY dir_path TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_nopad_bin NOT NULL',
          'media.file_path': 'ALTER TABLE media MODIFY file_path VARCHAR(1000) CHARACTER SET utf8mb4 COLLATE utf8mb4_nopad_bin NOT NULL',
        };
        await conn.query(definitions[`${column.table_name}.${column.column_name}`]);
      }
    });
...
async function clearAll() {
  return require('./lib/maintenance').withMaintenance(async () => {
    const conn = await pool.getConnection();
    try {
      await conn.beginTransaction();
      // Cascades remove social rows and tag links with FK checks left enabled.
      await conn.query('DELETE FROM media');
      await conn.query('DELETE FROM performers');
      await conn.query('DELETE FROM tags');
      await conn.commit();
    } catch (error) {
      try { await conn.rollback(); } catch (_) { conn.destroy(); }
      throw error;
...
async function batchInsertMedia(records) {
  ...
  return require('./lib/maintenance').withMaintenance(async () => {
    const unique = [...new Map(records.map(record => [record[2], record])).values()];
    const [existing] = await pool.query('SELECT file_path FROM media WHERE file_path IN (?)', [unique.map(record => record[2])]);
    const paths = new Set(existing.map(row => row.file_path));
    const pending = unique.filter(record => !paths.has(record[2]));
    if (!pending.length) return 0;
    const placeholders = pending.map(() => '(?, ?, ?, ?, ?, ?, ?, ?, ?)').join(', ');
    const [result] = await pool.query(
      `INSERT INTO media (performer_id, filename, file_path, type, mime_type, size, width, height, duration)
       VALUES ${placeholders}`, pending.flat()
    );
...
async function setResetToken(userId, token, expires) {
  await pool.query('UPDATE users SET reset_token = ?, reset_expires = ? WHERE id = ?', [hashResetToken(token), expires, userId]);
}

async function consumeResetToken(token, newHash) {
  const [result] = await pool.query(
    `UPDATE users SET reset_token = NULL, reset_expires = NULL, password_hash = ?,
      session_version = session_version + 1 WHERE reset_token = ? AND reset_expires > NOW()`,
    [newHash, hashResetToken(token)]
  );
  return result.affectedRows === 1;
}

async function changeUserPassword(userId, oldHash, newHash, sessionVersion) {
  const [result] = await pool.query(
    `UPDATE users SET reset_token = NULL, reset_expires = NULL, password_hash = ?,
      session_version = session_version + 1 WHERE id = ? AND password_hash = ? AND session_version = ?`,
    [newHash, userId, oldHash, sessionVersion]
  );
  return result.affectedRows === 1;
}

async function revokeUserSessions(userId, sessionVersion) {
  await pool.query('UPDATE users SET session_version = session_version + 1 WHERE id = ? AND session_version = ?',
...
async function withUserLock(operation) {
  const conn = await pool.getConnection();
  let acquired = false;
  let transaction = false;
  try {
    const [[lock]] = await conn.query("SELECT GET_LOCK(CONCAT('xflix_users:', LEFT(SHA2(DATABASE(), 256), 32)), 5) AS acquired");
    acquired = lock.acquired === 1;
    if (!acquired) throw Object.assign(new Error('Account management is busy'), { status: 503 });
    await conn.beginTransaction();
    transaction = true;
    ...
    if (acquired) {
      try { await conn.query("SELECT RELEASE_LOCK(CONCAT('xflix_users:', LEFT(SHA2(DATABASE(), 256), 32)))"); }
      catch (_) { conn.destroy(); }
    }
    conn.release();
  }
}

async function mutateUser(userId, role, actorId) {
  ...
  if (role !== null && !['admin', 'member'].includes(role)) throw Object.assign(new Error('Invalid role'), { status: 400 });
  return withUserLock(async conn => {
    const [[actor]] = await conn.query('SELECT role FROM users WHERE id = ? FOR UPDATE', [actorId]);
    if (actor?.role !== 'admin') throw Object.assign(new Error('Admin access required'), { status: 403 });
    const [[target]] = await conn.query('SELECT role FROM users WHERE id = ? FOR UPDATE', [userId]);
    if (!target) throw Object.assign(new Error('User not found'), { status: 404 });
    if (userId === actorId && role !== 'admin') throw Object.assign(new Error('Cannot remove your own admin access'), { status: 400 });
    const [admins] = await conn.query("SELECT id FROM users WHERE role = 'admin' FOR UPDATE");
    if (target.role === 'admin' && role !== 'admin' && admins.length <= 1) {
      throw Object.assign(new Error('At least one administrator must remain'), { status: 400 });
    }
    if (role === null) await conn.query('DELETE FROM users WHERE id = ?', [userId]);
    else if (role !== target.role) await conn.query(
      'UPDATE users SET role = ?, session_version = session_version + 1 WHERE id = ?', [role, userId]);
  });
}

async function updateUserRole(userId, role, actorId) { return mutateUser(userId, role, actorId); }
async function deleteUser(userId, actorId) { return mutateUser(userId, null, actorId); }
